...
- 本專案已建立 `.codegraph/` 索引；理解或定位程式碼時，優先直接呼叫
  `mcp__codegraph__codegraph_explore`。
- 每次工作階段開始時，不要預先檢查 CodeGraph CLI、`PATH`、daemon 或 MCP
  是否就緒，也不要因 PowerShell 找不到 `codegraph` 命令而判定 CodeGraph
  無法使用。
- 若 MCP 工具尚未顯示，從可用的延遲工具中載入
  `mcp__codegraph__codegraph_explore`，然後直接呼叫；MCP 才是 Codex 的標準
  使用入口，CLI 是否安裝與此無關。
- 只有 MCP 實際呼叫回傳錯誤或不可用時，才改用 `rg`／讀檔，並簡短記錄
  降級原因；不要為此要求完整檔案系統存取權或自行安裝 CLI。
...
- **好友群組與個人標記**: 結合 Firebase 提供登入機制，可標記想看的賽事、
  加入好友群組共享意願。在「我的標記」功能中支援兩階段延遲載入篩選
  （個人/群組/特定群友）、同日賽事多位成員意向彙整，並支援卡片式響應
  排版與一鍵匯出至 Google Calendar。
...
- **樣式**: SCSS (全域樣式集中於 `src/styles/` 目錄，各元件局部樣式於
  Vue 單一元件檔內的 `<style lang="scss" scoped>`)，無使用 Tailwind CSS
...
6. **狀態與資料流**: 無使用 Vuex 或 Pinia，狀態多半在根元件或透過
    Props / Events / Provide / Inject 來傳遞，或是直接與 Firebase 互動。
...
3. **測試方法**: 執行這些腳本時，通常使用 Node.js 直接運行
    （例：`node test-browser.js`），新增功能時，可建立類似的獨立腳本來
    驗證資料或 Firebase 連線。
...
1. **爬蟲功能**:
    ...
    - 開發環境中，爬蟲邏輯被整合在 `vite.config.js` 的 Vite custom plugin
      (`cpbl-scraper`) 內。請注意 Puppeteer 的 headless 設定與 Node.js
      執行環境。
2. **Firebase 整合**:
